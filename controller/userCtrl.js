...
const userCart = asyncHandler(async (req, res) => {
  const { itemId, price, images, title } = req.body;
...
const removeItemFromCart = asyncHandler(async (req, res) => {
  ...
  try {
    const deleteItemFromCart = await Cart.deleteOne({userId: _id, _id:cartItemId})
...
const createOrder = asyncHandler(async (req, res) => {
  ...
  try{
    const order = await Order.create({
      shippingInfo, paymentInfo, orderItems, totalPrice, user:_id
    })
    res.json({
      order,
      success:true
  ...
  const data = await Order.aggregate([
    ...
    },
    {
      $group: {
        _id: {
          month: "$month"
        }, amount: {$sum: "$totalPrice"}, count: {$sum: 1}
      }
    }
...
const getYearlyTotalOrders = asyncHandler(async (req, res) => {
  ...
  const data = await Order.aggregate([
    ...
    {
      $group: {
        _id: null, count: {$sum: 1}, amount: {$sum: "$totalPrice"}
      }
    }
